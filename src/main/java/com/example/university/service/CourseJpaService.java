...
public class CourseJpaService implements CourseRepository {
    ...
    @Autowired
    private ProfessorJpaRepository professorJpaRespository;

    @Override
    public List<Course> getCourses() {
        List<Course> courseList = courseJpaRespository.findAll();
    ...
    public Course getCourseByid(int courseid) {
        try {
            Course course = courseJpaRespository.findById(courseId).get();
            return course;
        } catch (Exception e) {
    ...
    public Course addCourse(Course course) {
        ...
        try {
            ...
            professor = professorJpaRespository.findById(professorId).get();
    ...
    public Course updateCourse(int courseId, Course course) {
        try {
            Course newCourse = courseJpaRespository.findById(courseId).get();
            ...
            courseJpaRespository.save(newCourse);
            return newCourse;
        } catch (Exception e) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Wrong professorId");
        }
